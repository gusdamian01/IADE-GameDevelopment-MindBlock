...
// Constraint targets (LEVEL 3)
#define ORANGE_TARGET_SUM 14
#define PURPLE_TARGET_SUM 9
...
typedef struct {
    int targetSum;
    char pieceId;  // kept for compatibility, not used in this level
} ConstraintRegion;
...
// Three constraints: orange (sum), purple (sum), green (all equal)
ConstraintRegion orangeConstraint;
ConstraintRegion purpleConstraint;
// greenConstraint: uses custom logic (all values equal) – no targetSum
...
// Initialize constraints for level 3
void init_constraints(void) {
    // Orange = full top row of the 4x4 puzzle
    orangeConstraint.targetSum = ORANGE_TARGET_SUM;
    orangeConstraint.pieceId   = '\0';

    // Purple = left column rows 1..3 inside the 4x4
    purpleConstraint.targetSum = PURPLE_TARGET_SUM;
    purpleConstraint.pieceId   = '\0';

    // Green: 2x2 center block – uses equality, handled by greenTilesEqual()
}
...
void printMap(void) {
    for (int x = 0; x < MAP_ROWS; x++) {
        for (int y = 0; y < MAP_COLS; y++) {
            ...
            // 2) Constraint backgrounds: green, purple, then orange
...
// LEVEL 3 PIECES
// Designed so that:
// - A fills the top row (orange) with sum 14
// - B fills the purple column (col 0, rows 1..3) with sum 9
// - C fills the central 2x2 (green), all equal values
// - D fills the remaining 4 cells
void initPieces(void) {
    srand((unsigned)time(NULL));
    numPieces = 0;

    // Piece A – top row (0,0..3) in solved state
    // shape: horizontal line
    struct Piece A = {
        'A', 4,
        {{0,0},{0,1},{0,2},{0,3}},
        {3,2,4,5},    // sum = 14 (orange target)
        3, 3, true
    };

    // Piece B – purple column + one extra cell
    // target cells in solved state:
    // (1,0),(2,0),(3,0),(3,1)
    // local coords relative to (1,0) => { (0,0),(1,0),(2,0),(2,1) }
    struct Piece B = {
        'B', 4,
        {{0,0},{1,0},{2,0},{2,1}},
        {2,3,4,1},    // purple sum = 2+3+4 = 9
        3, 14, true
    };

    // Piece C – central 2x2 (green region)
    // cells: (1,1),(1,2),(2,1),(2,2)
    struct Piece C = {
        'C', 4,
        {{0,0},{0,1},{1,0},{1,1}},
        {2,2,2,2},    // all equal => satisfies green '=' constraint
        7, 3, true
    };

    // Piece D – remaining 4 cells: (1,3),(2,3),(3,2),(3,3)
    // local coords relative to (1,2) => (0,1),(1,1),(2,0),(2,1)
    struct Piece D = {
        'D', 4,
        {{0,1},{1,1},{2,0},{2,1}},
        {1,3,2,4},
...
// LEVEL 3: Orange region = entire top row of the 4x4 puzzle
bool is_tile_in_orange(int x, int y) {
    int px0, py0;
...
// LEVEL 3: Purple region = left column rows 1..3 inside the puzzle
bool is_tile_in_purple(int x, int y) {
    int px0, py0;
    get_puzzle_origin(&px0, &py0);
    // rows px0+1,px0+2,px0+3, col py0
    if ((x == px0+1 || x == px0+2 || x == px0+3) && y == py0) {
        return true;
    }
    return false;
}

// LEVEL 3: Green region = central 2x2 block
bool is_tile_in_green(int x, int y) {
    int px0, py0;
    get_puzzle_origin(&px0, &py0);
    // cells (px0+1,py0+1..+2) and (px0+2,py0+1..+2)
    if ((x == px0+1 || x == px0+2) &&
        (y == py0+1 || y == py0+2)) {
        return true;
    }
...
// LEVEL 3: sum over ALL pieces (constraints not tied to a single piece)
int sumTilesInOrange(void) {
    int sum = 0;
    for (int i = 0; i < numPieces; i++) {
        struct Piece p = pieces[i];
...
// Green '=' constraint: all tiles in green region must have the SAME value
bool greenTilesEqual(void) {
    bool hasValue = false;
    int ref = 0;
    ...
    // Require at least one tile in the region
    return hasValue;
}
...
bool constraintsSatisfied(void) {
    ...
    if (!greenTilesEqual()) return false;
    return true;
}
...
int main(void) {
    ...
    while (1) {
        ...
        // Victory: all tiles inside 4x4 puzzle area AND all three constraints satisfied
        if (allPiecesFitInPuzzleArea() && constraintsSatisfied()) {
            printMap();
            printf("🎉 Level Complete. All constraints satisfied!\n");
            break;
        }
