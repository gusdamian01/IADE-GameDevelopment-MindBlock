...
bool greenConstraintSatisfied(void)
{
    if (currentLevel != 3 && currentLevel != 5) return true;
...
bool pinkConstraintSatisfied(void)
{
    if (currentLevel != 4 && currentLevel != 5) return true;
...
bool constraintsSatisfied(void) {
    ...
    if (!greenConstraintSatisfied()) return false;
    if (!pinkConstraintSatisfied())  return false;
...
int main(void)
{
            ...
            if (event.type == SDL_EVENT_KEY_DOWN)
            {
                ...
                if (currentState == STATE_MENU) {
                    ...
                    if (key == SDLK_W || key == SDLK_UP) {
                        ...
                        else if (mainMenuSelection == 1) { currentState = STATE_SETTINGS; settingsSelection = 0; }
                        else if (mainMenuSelection == 2) { currentState = STATE_CONFIRM_EXIT; confirmSelection = 0; }
                    }
                }
                ...
                // ===== CONFIRM EXIT =====
                    ...
                    if (key == SDLK_W || key == SDLK_S ||
                        ...
                        if (confirmSelection == 0) running = 0;
    ...
    if (startButtonTexture)    { SDL_DestroyTexture(startButtonTexture);    startButtonTexture = NULL; }
    if (settingsButtonTexture) { SDL_DestroyTexture(settingsButtonTexture); settingsButtonTexture = NULL; }
    if (exitButtonTexture)     { SDL_DestroyTexture(exitButtonTexture);     exitButtonTexture = NULL; }
...
void apply_brightness_to_textures(void)
{
    ...
    for (int i = 0; i < texCount; ++i) {
        if (texList[i]) SDL_SetTextureColorMod(texList[i], mod, mod, mod);
    }

    for (int i = 0; i < 7; ++i) {
        if (valueTextures[i]) SDL_SetTextureColorMod(valueTextures[i], mod, mod, mod);
    }
}
...
void printMap(void)
{
    ...
    for (int x = 0; x < MAP_ROWS; x++)
    {
        for (int y = 0; y < MAP_COLS; y++)
        {
            ...
            if (pIdx != -1 && tIdx != -1)
            {
                ...
                if (tex != NULL) SDL_RenderTexture(renderer, tex, NULL, &valRect);
                else SDL_RenderTexture(renderer, floorTexture, NULL, &valRect);
                continue;
            }
    ...
    for (int x = 0; x < MAP_ROWS; x++)
    {
        for (int y = 0; y < MAP_COLS; y++)
        {
            ...
            if (is_tile_in_purple(x, y)) SDL_RenderTexture(renderer, constraintTexture1, NULL, &cRect);
            if (is_tile_in_orange(x, y)) SDL_RenderTexture(renderer, constraintTexture2, NULL, &cRect);
            if (is_tile_in_green(x, y))  SDL_RenderTexture(renderer, constraintTexture3, NULL, &cRect);
            if (is_tile_in_pink(x, y))   SDL_RenderTexture(renderer, constraintTexture4, NULL, &cRect);
        }
    }
...
void renderMainMenu(void)
{
    ...
    if (startButtonTexture)    SDL_RenderTexture(renderer, startButtonTexture, NULL, &rectStart);
    if (settingsButtonTexture) SDL_RenderTexture(renderer, settingsButtonTexture, NULL, &rectSettings);
    if (exitButtonTexture)     SDL_RenderTexture(renderer, exitButtonTexture, NULL, &rectExit);

    SDL_FRect selectedRect = rectStart;
    if (mainMenuSelection == 1) selectedRect = rectSettings;
    if (mainMenuSelection == 2) selectedRect = rectExit;
...
void renderSettings(void)
{
    ...
    if (settingsSelection == 1) lineBrightness[0] = '>';
...
bool is_tile_in_purple(int x, int y)
{
    ...
    if (currentLevel == 1) {
        ...
        if (x == px0 + 1 && (y == py0 + 0 || y == py0 + 1)) return true;
        if (x == px0 + 2 && y == py0 + 0) return true;
        return false;
    }
}
...
{
    if (currentLevel != 3 && currentLevel != 5) return false;
...
{
    if (currentLevel != 4 && currentLevel != 5) return false;
    ...
    if (currentLevel == 4) {
        ...
        if (x == px0 + 2 && y == py0 + 3) return true;
        if (x == px0 + 3 && (y == py0 + 2 || y == py0 + 3)) return true;
        return false;
    }
}
...
static void play_sound_effect(SoundEffect *sfx)
{
    if (!sfx || !sfx->loaded || !sfx->stream) return;
